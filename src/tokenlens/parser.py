...
- Assistant lines with ``isApiErrorMessage: true`` are locally generated
  ...
  Other ``<synthetic>`` assistant lines (local placeholders such as
  "No response requested.") are skipped entirely.
...
from .pricing import DEFAULT_TABLE, SYNTHETIC_MODEL, PricingTable
...
class _Builder:
    ...
    def _assistant(self, entry: Dict[str, Any]) -> None:
        ...
        if message.get("model") == SYNTHETIC_MODEL:
            # Locally generated by Claude Code (e.g. "No response
            # requested."), never sent to the API: not a turn.
            return
